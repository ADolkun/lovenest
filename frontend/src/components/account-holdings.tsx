...
function WalletValue({ wallet }: { wallet: AssetGroup }) {
  ...
  return (
    <span className="tabular-nums">
      ...
      {missing > 0 && <span className="ml-1 text-xs text-warning-foreground">{t('accountHoldings.partial', { count: missing })}</span>}
    </span>
  )
}

/** Holdings remain distinct from the cash ledger and provider account total. */
export function AccountHoldingsSummary({ account, wallets }: { account: Account; wallets: AssetGroup[] }) {
  const { t } = useTranslation()
  ...
  return linked.map((wallet) => (
    <div key={wallet.id} className={wallet.source === 'manual' ? 'my-1 text-sm font-semibold' : 'mt-1 text-xs'}>
      <Link to={`/assets?wallet=${encodeURIComponent(wallet.id)}`} className="text-primary hover:underline">
        {t('accountHoldings.holdings')}: <WalletValue wallet={wallet} />
      </Link>
      {wallet.source === 'manual' && canWrite && <button type="button" className="ml-auto block text-xs font-normal text-muted-foreground underline sm:ml-2 sm:inline-block" disabled={unlink.isPending} onClick={() => unlink.mutate(wallet.id)}>{t('accountHoldings.unlink')}</button>}
    </div>
  ))
}
...
function WalletLinkRow({ wallet, accounts }: { wallet: AssetGroup; accounts: Account[] }) {
  ...
  return (
    <li className="flex flex-col gap-3 py-3 sm:flex-row sm:items-center sm:justify-between">
      <div className="flex min-w-0 items-baseline justify-between gap-4 sm:flex-1">
        <Link to={`/assets?wallet=${encodeURIComponent(wallet.id)}`} className="truncate text-sm font-medium text-primary hover:underline">{wallet.name}</Link>
        <div className="shrink-0 text-sm font-semibold"><WalletValue wallet={wallet} /></div>
      </div>
      {canWrite && accounts.length > 0 && (
        <form className="flex min-w-0 items-center gap-2 sm:ml-4 sm:max-w-80" onSubmit={(event) => { event.preventDefault(); if (accountId) link.mutate() }}>
...
export function UnlinkedWallets({ wallets, accounts }: { wallets: AssetGroup[]; accounts: Account[] }) {
  ...
  return (
    <section id="unlinked-wallets" aria-labelledby="unlinked-wallets-heading" className="rounded-xl border border-border bg-card px-4 py-4 sm:px-5">
