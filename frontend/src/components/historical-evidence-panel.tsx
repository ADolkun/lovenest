...
import type { HistoryAsset, HistoryCollection, HistoryEvidence, HistoryRequest } from '@/types'
...
function assetIdentity(asset: HistoryAsset | string): string {
  ...
  if (asset.native) return `${asset.chain}:native`
  return asset.chain === 'solana' ? `${asset.chain}:${asset.token_program ?? 'unknown-program'}:${asset.mint ?? 'unknown-mint'}` : `${asset.chain}:${asset.contract ?? asset.mint ?? 'unknown-contract'}`
}
...
export function HistoricalEvidencePanel({ workspaceId, initial, connectionIds, addressKeys }: {
  ...
  const snapshotBound = (snapshot: NonNullable<NonNullable<HistoryEvidence['reconciliation']>[number]['opening_snapshot']>) => {
    const block = snapshot.block_number ?? snapshot.slot
    return `${block != null ? `${t('history.blockOrSlot', 'Block / slot')} ${block}` : moment(snapshot.time)}${snapshot.block_hash ? ` · ${mask(snapshot.block_hash)}` : ''} (${label(snapshot.position ?? 'unknown')})`
  }
  ...
  const selectedChain = chainsQuery.data?.find((chain) => chain.key === selected?.chain)
  const supported = Boolean(selectedChain?.historical_evidence && !['unsupported', 'unavailable'].includes(selectedChain.historical_evidence))
  ...
  const mutation = useMutation({
    ...
    onSuccess: (data, operation) => {
      ...
      if (operation.kind === 'export') {
        ...
        anchor.href = url
        anchor.download = 'historical-evidence.json'
  ...
  const invalidBlocks = [startBlock, endBlock].some((value) => value !== '' && (!/^\d+$/.test(value) || !Number.isSafeInteger(Number(value)))) || Boolean(startBlock && endBlock && Number(startBlock) > Number(endBlock))
  const canCollect = Boolean(canWrite && ownershipConfirmed && supported && !watchedQuery.isError && !invalidWindow && !invalidBlocks && (!suppliedAddresses.length || reviewed))
  const collect = (event: FormEvent) => {
    ...
    mutation.mutate({ kind: 'collect', generation: ++generation.current, request: {
      ...
      ...(selectedChain?.kind === 'evm' && startBlock !== '' ? { start_block: Number(startBlock) } : {}),
      ...(selectedChain?.kind === 'evm' && endBlock !== '' ? { end_block: Number(endBlock) } : {}),
      ...(suppliedAddresses.length ? { supplied_accounts: suppliedAddresses.map((address) => ({ ...(selected.chain === 'bitcoin' && address.startsWith('script:') ? { scriptpubkey: address.slice(7) } : { address }), owner: selected.address, reviewed: true as const })) } : {}),
    } })
  }
  ...
  return <div className="space-y-6">
    <Card><CardContent className="space-y-4 p-4 sm:p-5">
      <div className="space-y-1"><h2 className="font-semibold">{t('history.collectTitle', 'Collect owned wallet evidence')}</h2><p className="max-w-3xl text-sm text-muted-foreground">{t('history.intro', 'Collect supported Solana, Ethereum, Base, Polygon and Bitcoin history from connected wallets. Collection retains private evidence without changing holdings, trades or basis.')}</p></div>
      {!!chainsQuery.data?.length && <details><summary className="min-h-11 cursor-pointer text-sm font-medium">{t('history.capabilities', 'Capabilities by source and network')}</summary><div className="mt-2 max-w-full overflow-x-auto"><table className="w-full text-left text-sm"><thead><tr><th className="p-2">{t('history.network', 'Network')}</th>{['holdings', 'native_research', 'owned_history', 'internal_execution', 'conversions', 'snapshots', 'external_continuation'].map((key) => <th className="p-2 font-medium" key={key}>{label(key)}</th>)}</tr></thead><tbody>{chainsQuery.data.map((chain) => <tr key={chain.key} className="border-t border-border"><th className="p-2 font-medium">{chain.display_name}</th>{['holdings', 'native_research', 'owned_history', 'internal_execution', 'conversions', 'snapshots', 'external_continuation'].map((key) => <td key={key} className="p-2">{label(chain.capabilities?.[key] ?? 'unknown')}</td>)}</tr>)}</tbody></table></div></details>}
      {watchedQuery.isPending && <Skeleton className="h-10 w-full" />}
      {chainsQuery.isError && <Alert variant="warning"><span>{t('history.capabilityError', 'Source capabilities could not be loaded. Retry before starting a collection.')}</span><Button variant="outline" onClick={() => void chainsQuery.refetch()}>{t('common.retry')}</Button></Alert>}
      ...
      <form onSubmit={collect} className="space-y-4">
        ...
        {selectedChain?.kind === 'evm' && <div className="grid gap-3 sm:grid-cols-2"><div className="space-y-1.5"><Label htmlFor="history-start-block">{t('history.startBlock', 'Start block (optional)')}</Label><Input id="history-start-block" inputMode="numeric" value={startBlock} maxLength={16} onChange={(event) => { setStartBlock(event.target.value); invalidate() }} /></div><div className="space-y-1.5"><Label htmlFor="history-end-block">{t('history.endBlock', 'End block (optional)')}</Label><Input id="history-end-block" inputMode="numeric" value={endBlock} maxLength={16} onChange={(event) => { setEndBlock(event.target.value); invalidate() }} /></div>{invalidBlocks && <p role="alert" className="text-sm text-warning-foreground sm:col-span-2">{t('history.invalidBlocks', 'Enter nonnegative whole block numbers, with the end on or after the start.')}</p>}</div>}
        <p className="text-xs text-muted-foreground">{t('history.bounds', 'Both UTC endpoints are inclusive. Unknown timestamps remain a coverage gap. Only evidence meeting the network settlement policy contributes to settled quantities.')}</p>
        ...
        {selected && chainsQuery.isSuccess && !supported && <Alert variant="warning">{t('history.unsupported', 'Historical collection is unsupported or unavailable for this source. Saved evidence remains accessible.')}</Alert>}
        {invalidWindow && <Alert variant="warning">{t('history.invalidWindow', 'The start must be on or before the end.')}</Alert>}
        {(selected?.chain === 'solana' || selected?.chain === 'bitcoin') && <details className="border-t border-border pt-3"><summary className="cursor-pointer text-sm font-medium">{selected?.chain === 'bitcoin' ? t('history.bitcoinInventory', 'Reviewed owned addresses and scripts') : t('history.suppliedTitle', 'Reviewed historical token accounts')}</summary><div className="mt-3 space-y-3"><Label htmlFor="history-supplied">{selected?.chain === 'bitcoin' ? t('history.bitcoinSupplied', 'Owned addresses or script:hex values (spaces or commas)') : t('history.supplied', 'Historical token-account addresses (separated by spaces or commas)')}</Label><Input id="history-supplied" type={privacyMode ? 'password' : 'text'} value={supplied} maxLength={10000} onChange={(event) => { setSupplied(event.target.value); setReviewed(false); invalidate() }} /><label className="flex items-start gap-2 text-sm"><input type="checkbox" className="mt-1" checked={reviewed} onChange={(event) => { setReviewed(event.target.checked); invalidate() }} /><span>{selected?.chain === 'bitcoin' ? t('history.bitcoinReviewed', 'I reviewed ownership of these additional addresses or scripts. Unlisted change and mixed-owner inputs remain unresolved.') : t('history.reviewed', 'I reviewed these historical token-account addresses for the selected wallet. Missing or conflicting chain ownership evidence must remain unresolved.')}</span></label></div></details>}
    ...
    {evidence && result && <section aria-label={t('history.results', 'Historical evidence results')} className="space-y-5">
      ...
      <details className="border-y border-border py-3"><summary className="cursor-pointer text-sm font-medium">{t('history.limits', 'Collection limits and settlement anchor')}</summary><dl className="mt-3 grid gap-3 text-sm sm:grid-cols-2"><div><dt className="text-muted-foreground">{t('history.anchor', 'Pinned reference block or slot')}</dt><dd>{evidence.anchor?.slot ?? evidence.anchor?.height ?? evidence.anchor?.block_number ?? evidence.anchor?.number ?? unknown}</dd></div>{evidence.anchor?.finalized?.number != null && <div><dt className="text-muted-foreground">{t('history.finalizedBlock', 'Reported finalized block')}</dt><dd>{typeof evidence.anchor.finalized.number === 'string' && /^0x[0-9a-f]+$/i.test(evidence.anchor.finalized.number) ? BigInt(evidence.anchor.finalized.number).toString() : evidence.anchor.finalized.number}</dd></div>}{evidence.chain === 'base' && <div><dt className="text-muted-foreground">{t('history.l1Finality', 'L1 finality corroborated')}</dt><dd>{evidence.anchor?.l1_corroborated === true ? t('common.yes', 'Yes') : unknown}</dd></div>}{(['seconds', 'attempts', 'attempts_used', 'concurrency', 'pages_per_address', 'accounts', 'archive_bytes', 'payload_bytes'] as const).map((key) => <div key={key}><dt className="text-muted-foreground">{label(key)}</dt><dd>{typeof evidence.limits[key] === 'number' ? String(evidence.limits[key]) : unknown}</dd></div>)}</dl></details>
      ...
      <details className="border-y border-border py-3"><summary className="cursor-pointer text-sm font-medium">{t('history.accounts', 'Account inventory and retrieval')} · {Object.keys(evidence.inventory).length}</summary><div className="mt-3 divide-y divide-border">{Object.entries(evidence.inventory).map(([address, account]) => {
        const streams = Object.entries(evidence.streams).filter(([key, stream]) => key === address || stream.address === address || (account.scriptpubkey != null && stream.scriptpubkey === account.scriptpubkey) || (['ethereum', 'base', 'polygon'].includes(evidence.chain) && evidence.owner === address))
        return <div key={address} className="space-y-2 py-3 text-sm"><p className="break-all font-mono">{mask(address)}</p><p>{label(account.kind)}{account.discoveries && <> · {account.discoveries.length} {t('history.discoveryRefs', 'discovery references')}</>} · {account.ownership.length} {t('history.ownershipRefs', 'ownership observations')}</p>{streams.length ? streams.map(([key, stream]) => <div key={key} className="space-y-1 border-l border-border pl-3">{stream.kind && <p className="font-medium">{label(stream.kind)}</p>}<p>{t('history.pages', 'Pages examined')}: {stream.pages_examined} · {t('history.exhausted', 'Provider exhausted')}: {stream.exhausted ? t('common.yes', 'Yes') : t('common.no', 'No')}</p><p>{t('history.observed', 'Observed bounds')}: {moment(stream.oldest_at)} → {moment(stream.newest_at)}</p><p>{t('history.unknownTimes', 'Unknown timestamps')}: {stream.unknown_timestamps}{stream.payload_gaps && <> · {t('history.payloadGaps', 'Payload gaps')}: {stream.payload_gaps.length}</>}</p>{stream.gaps?.map((gap) => <p key={gap}>{label(gap)}</p>)}{stream.stop_reason && <p>{label(stream.stop_reason)}</p>}{stream.cursor != null && <p className="break-all">{t('history.cursor', 'Resume cursor')}: {mask(typeof stream.cursor === 'string' ? stream.cursor : JSON.stringify(stream.cursor))}</p>}</div>) : <p>{t('history.unsearched', 'This account has not been searched.')}</p>}</div>
      })}</div></details>
      <div className="space-y-3"><h3 className="font-semibold">{t('history.reconciliation', 'Quantity reconciliation')}</h3><p className="text-sm text-muted-foreground">{t('history.equation', 'Opening + signed settled changes = closing. Missing or incompatible snapshots remain unknown; quantity equality does not certify history or basis.')}</p>{evidence.reconciliation?.length ? evidence.reconciliation.map((row, index) => <div key={index} className="space-y-1 border-b border-border py-3 text-sm"><p className="break-all">{mask(row.account)} · {mask(assetIdentity(row.asset))}</p><p className="break-words tabular-nums">{mask(row.opening ?? unknown)} + ({mask(row.settled_change ?? unknown)}) = {mask(row.closing ?? unknown)}</p><p>{t('history.discrepancy', 'Discrepancy')}: {mask(row.discrepancy ?? unknown)} · {label(row.status)}</p>{row.known_settled_change != null && <p>{t('history.knownSubtotal', 'Known settled subtotal')}: {mask(row.known_settled_change)}</p>}{row.scope && <p>{t('history.quantityScope', 'Quantity scope')}: {label(row.scope)} · {t('history.requestedInterval', 'Requested interval')}: {label(row.requested_interval_status ?? 'unknown')}</p>}{row.opening_snapshot && row.closing_snapshot && <p>{t('history.snapshotBounds', 'Snapshot bounds')}: {snapshotBound(row.opening_snapshot)} → {snapshotBound(row.closing_snapshot)}</p>}{row.reasons.length > 0 && <p>{row.reasons.map(label).join(' · ')}</p>}</div>) : <p className="text-sm">{t('history.noSnapshots', 'Compatible opening and closing snapshots are unavailable. Reconciliation remains unknown.')}</p>}</div>
