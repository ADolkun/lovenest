...
import { accountBalance, accountBasis, holdingComponents, holdingsMatchSnapshot, knownAmount, scopedExplanation } from '@/lib/balance-explanation'
...
export function AccountBalanceBasis({ account, workspaceId }: { account: Account; workspaceId?: string }) {
  ...
  const incomplete = accountBalance(account, workspaceId) === null || (account.type === 'investment' && (!detail || detail.coverage !== 'complete' || detail.refresh_status !== 'active'))
...
export function BalanceDetails({ account, wallets = [], holdings, holdingIds, holdingsError = false, children, autoOpen = false, workspaceId, canOpenAccounts = true, canOpenAssets = true, canWrite = false }: Props) {
  ...
  const activeHoldings = (holdings ?? []).filter((holding) => !holding.sell_date && !holding.is_archived)
  if (!account && (rows.length === 0 || rows.some((row) => !row.invalid))) {
    if (holdingIds && activeHoldings.length > 0) rows.push({ id: 'selected-holdings', name: t('balanceExplanation.selectedHoldings'), detail: null })
    else if (activeHoldings.some((holding) => holding.group_id === null)) rows.push({ id: 'unassigned', name: t('assets.noWallet'), detail: null })
  }
  const currentHoldings = (id: string) => activeHoldings.filter((holding) => account || id === 'selected-holdings' || (id === 'unassigned' ? holding.group_id === null : holding.group_id === id))
  const fallbackHoldings = (id: string): BalanceHolding[] => currentHoldings(id).map((holding) => ({ asset_id: holding.id, wallet_id: holding.group_id, name: holding.name, ticker: holding.ticker, type: holding.type, quantity: holding.units, value: holding.current_value, currency: holding.currency, observed_at: holding.last_price_at ?? holding.value_updated_at, observation_basis: holding.last_price_at ? 'quote' : holding.value_updated_at ? 'recorded' : 'unknown' }))
      ...
      <DialogContent className="max-w-2xl">
        ...
        <div className="divide-y divide-border">
          {rows.filter((row) => !row.invalid).map(({ id, name, detail }) => {
            const snapshotMismatch = !!detail && holdings !== undefined && !holdingsError && !holdingsMatchSnapshot(detail.holdings, currentHoldings(id))
            const items = (detail && !snapshotMismatch ? detail.holdings : fallbackHoldings(id)).filter((holding) => !holdingIds || holdingIds.includes(holding.asset_id))
            ...
            return <section key={id} aria-label={name} className="space-y-4 py-5 first:pt-0 last:pb-0">
              ...
              {snapshotMismatch && <p role="status" className="text-sm text-warning-foreground">{t('balanceExplanation.snapshotMismatch')}</p>}
              ...
              {detail && !holdingIds && <>
                <dl className="space-y-3 text-sm">
                  ...
                  <div className="flex flex-wrap gap-x-8 gap-y-3"><div><dt className="text-muted-foreground">{t('balanceExplanation.refresh')}</dt><dd>{t(`balanceExplanation.refreshState.${detail.refresh_status}`, { defaultValue: t('balanceExplanation.refreshState.unknown') })}</dd><dd className="mt-1 text-xs text-muted-foreground">{age(detail.refresh_observed_at)}</dd></div><div><dt className="text-muted-foreground">{t('balanceExplanation.coverage')}</dt><dd>{snapshotMismatch ? t('balanceExplanation.coverageIncomplete') : t(`balanceExplanation.coverageState.${detail.coverage}`)}</dd></div></div>
                ...
                <p className="text-sm text-muted-foreground">{t(`balanceExplanation.reconciliation.${snapshotMismatch ? 'not_comparable' : detail.reconciliation}`)}</p>
                {!snapshotMismatch && detail.reconciliation === 'difference' && <p className="text-sm font-medium">{t('balanceExplanation.difference')}: <span className="tabular-nums">{money(detail.difference, detail.currency)}</span></p>}
              ...
              {detail && !holdingIds && <dl className="text-sm"><div className="flex flex-wrap justify-between gap-2"><dt className="text-muted-foreground">{t('balanceExplanation.component.liquid_cash')}</dt><dd className="tabular-nums">{money(holdingsError || snapshotMismatch ? null : detail.residual_cash, detail.holdings_currency)} {detail.holdings_currency}</dd></div></dl>}
              {items.length > 0 && <div>
