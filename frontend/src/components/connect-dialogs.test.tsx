...
const api = vi.hoisted(() => ({ getProviders: vi.fn(), listInstitutions: vi.fn(), getReconnectToken: vi.fn(), getConnectToken: vi.fn(), toastError: vi.fn(), listPasskeys: vi.fn() }))
...
it('resets a token and initial sync choices when reopened', async () => {
  const props = { onClose: vi.fn(), provider: 'simplefin', supportsAssetSync: true }
  const { user, rerender } = renderWithProviders(<TokenConnectDialog open {...props} />)
  await user.type(screen.getByRole('textbox'), 'synthetic-token')
  const syncAssets = document.querySelector<HTMLInputElement>('#token-sync-assets')!
  await user.click(syncAssets)
  expect(syncAssets).not.toBeChecked()
  rerender(<TokenConnectDialog open={false} {...props} />)
  rerender(<TokenConnectDialog open {...props} />)
  expect(screen.getByRole('textbox')).toHaveValue('')
  expect(document.querySelector('#token-sync-assets')).toBeChecked()
})

it('keeps the current passkey list when a closed session finishes loading', async () => {
  let finish!: (data: unknown) => void
  api.listPasskeys.mockImplementationOnce(() => new Promise((resolve) => { finish = resolve }))
    .mockResolvedValueOnce([{ id: 'new', name: 'New passkey', created_at: '2026-01-01T00:00:00Z', last_used_at: null }])
  const onClose = vi.fn()
  const { rerender } = renderWithProviders(<PasskeyManagementDialog open onClose={onClose} />)
  rerender(<PasskeyManagementDialog open={false} onClose={onClose} />)
  rerender(<PasskeyManagementDialog open onClose={onClose} />)
  await screen.findByText('New passkey')
  await act(async () => { finish([{ id: 'old', name: 'Old passkey', created_at: '2026-01-01T00:00:00Z', last_used_at: null }]) })
  expect(screen.queryByText('Old passkey')).not.toBeInTheDocument()
