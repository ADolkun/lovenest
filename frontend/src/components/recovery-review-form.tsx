...
export function RecoveryReviewForm({ entry, data, busy, onSave }: { entry: RecoveryEntryRead; data: RecoveryPackage; busy: boolean; onSave: (review: RecoveryReviewInput) => void }) {
  ...
  const [relationKind, setRelationKind] = useState('claim_notice')
  const documentary = kind === 'relation' && relationKind.startsWith('documentary_')
  ...
  const transfers = useQuery({ queryKey: ['owned-transfers', data.workspace_id], queryFn: ({ signal }) => ownedTransfers.index(data.workspace_id, signal), enabled: kind === 'relation' && !documentary, retry: false })
  ...
  const submit = (event: FormEvent<HTMLFormElement>) => {
    ...
    onSave({
      ...
      documentary_quantity: text('documentary_quantity'),
    })
  }
  ...
  return <form onSubmit={submit} className="space-y-4 border-t border-border pt-4">
    <fieldset disabled={busy} className="space-y-4">
      ...
      <div key={kind} className="grid gap-4 sm:grid-cols-2">
        {kind === 'relation' && <>
          <div className="space-y-1.5"><Label htmlFor={`${entry.id}-relation_kind`}>{t('recovery.relationship', 'Relationship')}</Label><NativeSelect className={selectClass} id={`${entry.id}-relation_kind`} name="relation_kind" value={relationKind} onChange={(event) => setRelationKind(event.target.value)}>{['claim_notice', 'notice_receipt', 'receipt_disposition', 'disposition_proceeds', 'candidate_acquisition', 'owned_transfer_reference', 'documentary_equity_distribution', 'documentary_receipt_disposition'].map((value) => <option key={value} value={value}>{label(value)}</option>)}</NativeSelect></div>
          ...
          {documentary && <p className="text-sm text-muted-foreground sm:col-span-2">{t('recovery.documentaryHelp', 'Confirmation records a documented association only. It does not prove applied inventory, acquisition lots, basis, tax treatment or final claim settlement. Document the account, source dates and association in the review reason.')}</p>}
          {documentary && relationKind === 'documentary_receipt_disposition' && field('documentary_quantity', t('recovery.documentaryQuantity', 'Documented association quantity (same asset only)'))}
          {!documentary && <>{field('quantity_adjustment', t('recovery.adjustment', 'Reported rounding / fee adjustment (signed amount)'))}
