...
export function RecoveryEvidenceForm({ busy, observations, onPreview, onChange }: { busy: boolean; observations: EvidenceObservation[]; onPreview: (draft: RecoveryDraft) => void; onChange: () => void }) {
  ...
  return <form onSubmit={submit} onChange={onChange} className="space-y-5">
    <fieldset disabled={busy} className="space-y-5">
      ...
      {selected && <p className="text-sm text-muted-foreground">{t('recovery.attachHelp', 'Original source facts remain unchanged. Add only recovery context and separate reported details below.')}</p>}
      ...
      {(selected ? rows.slice(0, 1) : rows).map((row, index) => <fieldset key={row} className="space-y-3 border-t border-border pt-4">
        ...
        <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
          ...
          {roleFields[role].map(([name, title]) => field(`leg-${row}-${name}`, title, { type: name.endsWith('_date') ? 'date' : 'text' }))}
        </div>
