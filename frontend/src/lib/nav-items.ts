 ...
 */
export type NavItem =
  | { type: 'link'; key: string; path: string; icon: React.ElementType; module: ModuleId; external?: false }
  | { type: 'link'; key: string; path: string; icon: React.ElementType; external: true; module?: never }
...
export function visibleNavItems(
  ...
  const kept = items.filter(
    (item) => item.type !== 'link' || item.external || hasModule(item.module),
  )
