...
describe('visibleNavItems', () => {
  ...
  it('keeps always-visible external links and their section header', () => {
    const result = visibleNavItems(navItems, none)
    expect(linkKeys(result)).toEqual(['taxPlanning'])
    expect(result.filter((i) => i.type === 'separator').map((i) => i.labelKey)).toEqual([
      'nav.groupAnalysis',
    ])
  })
