...
@pytest.fixture
def invoice_today():
    """Upstream's name for the pinned service date; the autouse clock pins it."""
    return _TEST_TODAY
...
async def _create(client, headers, **overrides):
    payload = {"total": "3000.00", "due_date": str(_TEST_TODAY + timedelta(days=15))}
...
async def test_overdue_needs_no_job(client: AsyncClient, biz_headers, invoice_today):
    ...
    invoice = await _create(
        client,
        biz_headers,
        issue_date=str(_TEST_TODAY - timedelta(days=40)),
        due_date=str(_TEST_TODAY - timedelta(days=10)),
    )
...
async def test_moving_the_due_date_undoes_overdue_with_no_compensating_write(
    ...
    moved = await client.patch(
        ...
        headers=biz_headers,
        json={"due_date": str(_TEST_TODAY + timedelta(days=10))},
    )
...
async def test_transaction_from_another_workspace_is_not_found(
    ...
    foreign = Transaction(
        ...
        currency="USD", date=_TEST_TODAY, type="credit", source="manual",
    )
    session.add(foreign)
    await session.commit()
...
async def test_summary_buckets_and_excludes_drafts(client: AsyncClient, biz_headers):
    ...
    await _create(client, biz_headers, total="200.00",
                  due_date=str(_TEST_TODAY + timedelta(days=5)))
    await _create(client, biz_headers, total="300.00",
                  issue_date=str(_TEST_TODAY - timedelta(days=60)),
                  due_date=str(_TEST_TODAY - timedelta(days=45)))
...
async def test_competence_date_defaults_to_issue_but_can_diverge(
    ...
    delivered = str(_TEST_TODAY - timedelta(days=35))
...
async def test_due_date_defaults_to_payment_terms(
    ...
    assert resp.status_code == 201, resp.text
    assert resp.json()["due_date"] == str(_TEST_TODAY + timedelta(days=7))
...
async def test_a_personal_workspace_never_pays_for_the_badge(
    ...
    session.add(
        Transaction(
            ...
            currency="USD", date=_TEST_TODAY, type="debit", source="manual",
        )
    )
    await session.commit()
