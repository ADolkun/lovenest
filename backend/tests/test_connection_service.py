...
async def test_sync_holdings_mixed_hints_stay_stable_across_syncs(
    ...
    crashes on the unique (workspace, user, source, external_id) index
...
async def test_sync_holdings_never_touches_another_workspaces_wallets(
    ...
    # Lovenest scopes provider identities by workspace: the foreign archived
    # row stays put and this sync creates a separate current-workspace row.
    ...
    synced_asset = await session.scalar(
        select(Asset).where(
            Asset.external_id == "h-1",
    ...
    assert synced_asset.group_id == minted.id
    assert synced_asset.is_archived is False
...
async def test_wallet_key_uniqueness_is_enforced_in_the_test_schema(
    ...
    """The model mirrors migration 071's workspace-scoped partial index so adoption
...
@pytest.mark.asyncio
async def test_ensure_group_keeps_same_provider_key_separate_across_workspaces(
    session: AsyncSession, test_user, test_workspace,
):
    """The same provider key can represent separate links in two workspaces."""
    from app.models.asset_group import AssetGroup
    from app.models.workspace import Workspace, WorkspaceMember
    from app.services.asset_group_service import ensure_group_for_connection
