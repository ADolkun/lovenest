...
async def create(session, workspace, user, **data):
    payload = {
        "total": Decimal("1000.00"),
        # Pinned, not defaulted: create_invoice dates an invoice "today" when
        # the caller stays quiet, and a floating issue date eventually walks
        # past this fixed due date and fails every test in the file.
        "issue_date": TODAY,
        "due_date": TODAY + timedelta(days=10),
    }
    payload.update(data)
