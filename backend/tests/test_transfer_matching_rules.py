...
async def _same_day_pair(session: AsyncSession, test_user, ws):
    ...
    incoming leg lands two days later. All three carry one transfer
    reference, the evidence Lovenest requires before a pair is considered,
    so only the destination name can separate the two debits.
    """
    ...
    to_tr = await _txn(
        ...
        description="Transfer to Trade Republic 0987 REF 55501234",
    )
    to_fortuneo = await _txn(
        session, test_user.id, ws.id, bnp,
        amount="100.00", kind="debit", when=TODAY,
        description="Transfer to FORTUNEO ACCOUNT 1234 REF 55501234",
    )
    arrival = await _txn(
        session, test_user.id, ws.id, fortuneo,
        amount="100.00", kind="credit", when=TODAY + timedelta(days=2),
        description="Transfer from BNP REF 55501234",
    )
...
async def test_two_indistinguishable_legs_are_asked_about_rather_than_guessed(
    ...
    first = await _txn(
        ...
        amount="100.00", kind="debit", when=TODAY,
        description="VIREMENT TRANSFER REF 883200001",
    )
    second = await _txn(
        session, test_user.id, ws.id, source,
        amount="100.00", kind="debit", when=TODAY,
        description="VIREMENT TRANSFER REF 883300001",
    )
    await _txn(
        session, test_user.id, ws.id, target,
        amount="100.00", kind="credit", when=TODAY,
        description="RECU VIREMENT TRANSFER REF 00001",
    )
...
async def test_paying_the_card_bill_is_a_transfer_and_still_links(
    ...
    the plainest sense and people have always wanted it paired. Lovenest
    pairs it on the issuers' payment markers.
    """
    card = await _account(session, test_user.id, ws.id, "Card", kind="credit_card")
    checking = await _account(session, test_user.id, ws.id, "Checking")
    ...
    paid = await _txn(
        session, test_user.id, ws.id, checking,
        amount="1240.00", kind="debit", when=TODAY,
...
async def test_a_workspace_can_switch_transfer_pairing_off_entirely(
    ...
    debit = await _txn(
        session, test_user.id, ws.id, source,
        amount="50.00", kind="debit", when=TODAY, description="TRANSFER REF 50000001",
    )
    await _txn(
        session, test_user.id, ws.id, target,
        amount="50.00", kind="credit", when=TODAY, description="TRANSFER REF 50000001",
    )
...
async def test_a_sync_that_brought_nothing_new_pairs_nothing(
    ...
    debit = await _txn(
        session, test_user.id, ws.id, source,
        amount="75.00", kind="debit", when=TODAY, description="TRANSFER REF 75000001",
    )
    credit = await _txn(
        session, test_user.id, ws.id, target,
        amount="75.00", kind="credit", when=TODAY, description="TRANSFER REF 75000001",
    )
...
async def test_accepting_a_question_about_rows_that_changed_is_refused(
    ...
    await _txn(
        session, test_user.id, ws.id, source,
        amount="300.00", kind="debit", when=TODAY,
        description="VIREMENT TRANSFER REF 111100001",
    )
    await _txn(
        session, test_user.id, ws.id, source,
        amount="300.00", kind="debit", when=TODAY,
        description="VIREMENT TRANSFER REF 222200001",
    )
    arriving = await _txn(
        session, test_user.id, ws.id, target,
        amount="300.00", kind="credit", when=TODAY,
        description="RECU TRANSFER REF 00001",
    )
