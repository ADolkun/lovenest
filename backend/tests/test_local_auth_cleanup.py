"""Cleanup remains authenticated and verified even when local enrollment is off."""
...
async def test_disabled_local_auth_cleanup_requires_both_factors(
    ...
    invalid_code = next(f"{value:06d}" for value in range(10) if not totp.verify(f"{value:06d}", for_time=now, valid_window=1))
    ...
    assert response.json()["detail"] == ("Invalid password" if bad_factor == "password" else "Invalid 2FA code")
...
async def test_disabled_local_auth_passkey_cleanup_is_owner_scoped(
    ...
    own = UserPasskey(user_id=test_user.id, credential_id="own", public_key="synthetic", name="Own key")
    other = UserPasskey(user_id=test_user_with_2fa.id, credential_id="other", public_key="synthetic", name="Other key")
    ...
    assert (await client.delete(f"/api/auth/passkeys/{other.id}", headers=headers)).status_code == 404
    assert (await client.delete(f"/api/auth/passkeys/{own.id}", headers=headers)).status_code == 204
