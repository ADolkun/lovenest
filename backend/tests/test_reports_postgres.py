...
async def test_income_expenses_weekly_iso_year_boundary(
    ...
    for when, amount in [(date(2020, 12, 31), 10), (date(2021, 1, 1), 20), (date(2021, 1, 4), 40)]:
        session.add(Transaction(
            user_id=test_user.id, account_id=test_account.id,
            description="Synthetic year-boundary income", amount=Decimal(amount),
            amount_primary=Decimal(amount), currency="BRL", date=when, type="credit", source="manual",
        ))
    await session.commit()
    response = await client.get(
        "/api/reports/income-expenses", params={"months": 1, "interval": "weekly"}, headers=auth_headers,
    )
    assert response.status_code == 200
    data = response.json()
    buckets = {point["date"]: point["breakdowns"]["income"] for point in data["trend"]}
    assert buckets["2020-W53"] == 30
    assert buckets["2021-W01"] == 40
    assert sum(buckets.values()) == 70
    assert next(b["value"] for b in data["summary"]["breakdowns"] if b["key"] == "income") == 70
