...
class TestBillTotalIgnoresReportingExclusions:
    ...
    @pytest.mark.asyncio
    async def test_transfer_like_refund_nets_bill_without_becoming_income(
        self, session, test_user, test_workspace, cc_account, transfer_category
    ):
        """Lovenest classifies card credits as refunds, not income, while
        still netting them against the amount owed on the bill."""
        await _make_tx(
            session, test_user.id, cc_account.id,
            date(2026, 4, 3), Decimal("100"), tx_type="debit",
        )
