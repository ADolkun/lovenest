...
def _has_transfer_evidence(
    a: Transaction, b: Transaction, accounts: dict[uuid.UUID, Account]
) -> bool:
    """Lovenest's gate: both descriptions must corroborate one transfer.

    Equal amounts on nearby days are common between unrelated rows on US
    statements (migration 067 unpaired the ones the old detector linked), so
    an automatic link also needs a shared card issuer or a shared provider
    reference. Without it the pair is never a candidate: accepting a
    suggestion re-checks amounts and accounts, not meaning, so asking about
    an uncorroborated pair invites a wrong link.
    """
    debit, credit = (a, b) if a.type == "debit" else (b, a)
    debit_account = accounts.get(debit.account_id)
    credit_account = accounts.get(credit.account_id)
    debit_type = debit_account.type if debit_account else ""
    credit_type = credit_account.type if credit_account else ""
    debit_name = debit_account.name if debit_account else ""
    credit_name = credit_account.name if credit_account else ""
    if is_credit_card_payment(
        debit.description, debit.type, debit_type
    ) or is_credit_card_payment(credit.description, credit.type, credit_type):
    ...
    return is_compatible_non_card_transfer(
        debit.description, credit.description, debit_name, credit_name
    )
            ...
            for other in by_date.get(tx.date + timedelta(days=offset), ()):
                ...
                if ratio is not None:
                    ...
                    if abs(moved - theirs) > (theirs * ratio):
                        continue
                if not _has_transfer_evidence(tx, other, accounts):
                    continue
                found.append(other)
        return found
