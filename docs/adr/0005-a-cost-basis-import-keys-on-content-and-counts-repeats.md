...
against the same row — which is also why migration 082 widens that scale to
eighteen decimals, since a crypto lot report routinely carries sixteen. And a
...
matches `manual` alongside `market_price`, or the next import of the same
ticker would build a second holding beside the first.
